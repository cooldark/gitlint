...
class ConfigTests(BaseTestCase):
    ...
    def test_ignore_by_id(self):
        ...
        output = gitlint("--ignore", "T5,B4", _cwd=self.tmp_git_repo, _ok_code=[1])
    ...
    def test_ignore_by_name(self):
        ...
        output = gitlint("--ignore", "title-must-not-contain-word,body-first-line-empty",
                         _cwd=self.tmp_git_repo, _ok_code=[1])
        expected = u"1: T3 Title has trailing punctuation (.): \"WIP: Thïs is a title.\"\n"
        self.assertEqual(output.stderr, expected)

    def test_verbosity(self):
        self._create_simple_commit(u"WIP: Thïs is a title.\nContënt on the second line")
        output = gitlint("-v", _cwd=self.tmp_git_repo, _ok_code=[3])
        expected = u"1: T3\n1: T5\n2: B4\n"
        self.assertEqual(output.stderr, expected)

        output = gitlint("-vv", _cwd=self.tmp_git_repo, _ok_code=[3])
        ...
        output = gitlint("-vvv", _cwd=self.tmp_git_repo, _ok_code=[3])
        expected = u"1: T3 Title has trailing punctuation (.): \"WIP: Thïs is a title.\"\n" + \
        ...
        # test silent mode
        output = gitlint("--silent", _cwd=self.tmp_git_repo, _ok_code=[3])
        self.assertEqual(output.stderr, "")

    def test_set_rule_option(self):
        self._create_simple_commit(u"This ïs a title.")
        output = gitlint("-c", "title-max-length.line-length=5", _cwd=self.tmp_git_repo, _ok_code=[3])
    ...
    def test_config_from_file(self):
        ...
        output = gitlint("--config", config_path, _cwd=self.tmp_git_repo, _ok_code=[5])
    ...
    def test_config_from_file_debug(self):
        ...
        config_path = self.get_sample_path("config/gitlintconfig")
        output = gitlint("--config", config_path, "--debug", _cwd=self.tmp_git_repo, _ok_code=[5])
